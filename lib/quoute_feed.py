...
class QuoteFeedFactory:
    @staticmethod
    def create(isin: str, data_type: str, value: float, quote_feed_id: str) -> QuoteFeed:
        return {
            "isin": isin,
            "data_type": data_type,
            "value": value,
    ...
    @staticmethod
    def bulk_create(isin: str, data_type: str, values: List[float], quote_feed_id: str) -> List[QuoteFeed]:
...
def post_quote_feed_without_sleep(payload:QuoteFeed, fmt: bool = True):
    try:
        data_as_json = json.dumps(payload)
        r =  requests.post(url=URL_PLUTUS, data=data_as_json, headers=HEADERS)
